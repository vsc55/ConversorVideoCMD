...
### UPDATE
  ...
  - Menús del perfil custom: **todos** los que tienen valor por defecto marcan la opción con `<= por defecto` (encoder, perfil de codec, level y bitrate de audio), además del `[N]` en el prompt. `Select-FromList` marca la opción por defecto (o `0. ninguno` si el default no aplica al codec). El menú de **encoder** también se numera automáticamente (1..N desde una lista), sin claves a mano. Si `customProfile.videoEncoder` trae un encoder desconocido (errata en config), el default cae al de la app (`hevc_nvenc`), no a la primera opción.
