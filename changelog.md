...
### NEW
  ...
  - `config.json` ORGANIZADO POR SECCIONES: `downloads`, `languages`, `encode`, `border`, `volume`, `behavior`, `console` (FUSION PROFUNDA CON LOS VALORES POR DEFECTO, AMPLIABLE SIN ROMPER CONFIGS EXISTENTES).
  ...
  - UTILIDAD `setup.ps1` (+ `setup.cmd`, LANZADOR CON BYPASS):
    - GESTION DE HERRAMIENTAS: INSTALAR / CAMBIAR VERSION, REINSTALAR (POR CARPETA DE VERSION), REINSTALAR TODO, Y ESTADO CON LAS VERSIONES REALMENTE INSTALADAS POR PLATAFORMA.
    - EDITOR NAVEGABLE DE TODO `config.json` SIN TOCARLO A MANO (SELECTORES DE COLOR/METODO/BOOLEANOS, EDITOR DE LISTAS), CON GUARDADO PROPIO QUE CONSERVA VALORES, TIPOS, ARRAYS Y FORMATO (4 ESPACIOS, CRLF).
    - LIMPIEZA DE `Proceso`: JOBS (`*.job.json`), BLOQUEOS (`*.lock`) Y TEMPORALES, CON CONFIRMACION.
    - CHECKLIST DE LOS DIRECTORIOS DE TRABAJO (LOS CREA SI FALTAN).
...
### REFACTOR
  - TODA LA GESTION DE APPS/VERSIONES/DESCARGAS SE SACA DE `Common.psm1` A UN MODULO PROPIO `lib\Tools.psm1`.
  - ELIMINACION DE CODIGO DUPLICADO CON FUENTES UNICAS DE VERDAD: `Get-CvWorkDirs` (CARPETAS DE TRABAJO), `Get-CvAppDescriptor` (DESCRIPTOR DE APP), `New-CvToolContext` (RUTAS Y NOMBRES DE EXE), `Get-CvToolDir` (CARPETA VERSION/PLATAFORMA) Y `Get-CvTempPaths` (RUTAS DE FICHEROS TEMPORALES).
