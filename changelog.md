...
## VERSION 4.0 - 07/07/2026
### NEW
  - REESCRITURA COMPLETA A POWERSHELL 5.1, EN PARALELO AL BATCH (EL `.cmd` SE MANTIENE). DISENO MODULAR EN `lib\` Y CONFIGURACION EN `config.json`.
  - NUEVA ESTRUCTURA:
    - `LimpiarBorde.ps1` -> ORQUESTACION (CLASIFICAR / PREPARAR / WORKER CON MUTEX).
    - `run.cmd` -> LANZADOR (`-ExecutionPolicy Bypass` + `chcp 65001` PARA UTF-8).
    - `config.json` -> TODA LA CONFIGURACION, SE CARGA AL ARRANCAR.
    - `lib\Common.psm1` (contexto, config, herramientas, jobs JSON, lock, consola), `lib\MediaInfo.psm1` (ffprobe JSON), `lib\Profile.psm1`, `lib\Video.psm1`, `lib\Audio.psm1`, `lib\Subtitle.psm1`, `lib\Multiplex.psm1`.
  ...
  - GESTION DE SUBTITULOS POR IDIOMA: MANTIENE EL COMPLETO + LOS FORZADOS DEL IDIOMA PREFERIDO, CON SUS METADATOS (`language`, `default`, `forced`).
