...
| **OnePass** | 🧪 BETA. Ejecución en UNA sola pasada de ffmpeg: `Test-CvOnePassEligible` (decide si el job puede fundirse en un comando, con el motivo si no), `Get-CvOnePassArgs` (constructor **puro** del comando: un `-i` + `-filter_complex` con la rama de vídeo `crop→scale` y una rama por pista de audio `adelay→downmix→loudnorm`, más el mapeo de subtítulos/adjuntos/capítulos del original —espejo, fundido, de Invoke-VideoRun + Invoke-AudioRun + Invoke-Multiplex) e `Invoke-CvOnePass` (ejecuta con progreso en vivo y limpia etiquetas). El worker lo usa cuando es elegible (activador `test.betaOnePass`); si no, cae al pipeline por etapas. |
